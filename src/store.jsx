...
// Dynamically pull the API base URL from the environment, falling back to localhost if missing.
...
export const AppProvider = ({ children }) => {
  ...
  // useCallbacks to prevent infinite re-renders inside children useEffects
  ...
  useEffect(() => {
    fetchDocuments(); fetchMetrics(); fetchLogs(); fetchHealth(); fetchSettings();
  }, [fetchDocuments, fetchMetrics, fetchLogs, fetchHealth, fetchSettings]);
  ...
  useEffect(() => {
    ...
    if (hasPending) {
      const intervalId = setInterval(() => {
        fetchDocuments(); fetchMetrics(); fetchHealth();
      }, 3000); 
      return () => clearInterval(intervalId); 
    }
  }, [documents, fetchDocuments, fetchMetrics, fetchHealth]);

  // OPTIMIZATION: useMemo prevents the context provider from forcing a re-render 
  // on every child component unless these specific values actually change.
